...
procedure TfrmChildDish.btnClearInputFieldClick(Sender: TObject);
begin
  if not DMVFP.TblDBdish.Eof then
  begin
    DMVFP.TblDBdish.First;
  end;
  edtInputCodeRecord.Text := '';
  DBGrid2.DataSource := DMVFP.DsDBdish;
end;

procedure TfrmChildDish.btnGoToRecordClick(Sender: TObject);
var
...
begin
  ...
  begin
    with DMVFP do
    begin
      if not TblDBdish.Eof then
      begin
        TblDBdish.First;
        while not TblDBdish.Eof do
        begin
          if TblDBdish['code_d'] = codeForSearch then
          begin
            Break;
          end;
          TblDBdish.Next;
        ...
        if TblDBdish.Eof then
        begin
          TblDBdish.First;
          questionMB := 'Дані не знайдено за кодом "' + codeForSearch + '", перевірте коректність вводу (ввід у форматі "ХХХd", де Х - це цифра)';
